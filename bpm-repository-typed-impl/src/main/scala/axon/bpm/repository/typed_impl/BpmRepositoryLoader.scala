...
import axon.bpm.repository.typed_impl.bpmdiagram.{BpmDiagramElastic, BpmDiagramEntity, BpmDiagramEventProcessor, BpmDiagramRepository, BpmDiagramSerializerRegistry, BpmDiagramService}
...
abstract class BpmRepositoryApplication(context: LagomApplicationContext)
  ...
  clusterSharding.init(
    Entity(BpmDiagramEntity.typeKey) { entityContext =>
      BpmDiagramEntity(entityContext)
    }
  )
